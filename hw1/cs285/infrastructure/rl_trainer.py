...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(
        ...
        if itr == 0:
            loaded_paths = pkl.load(open("../../" + load_initial_expertdata, "rb"))
    ...
    def train_agent(self):
        ...
        for train_step in trange(self.params['num_agent_train_steps_per_iter']):
            ...
            ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch = \
                self.agent.replay_buffer.sample_random_data(batch_size=self.params['train_batch_size'])
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for i in range(len(paths)):
            expert_actions = expert_policy(ptu.from_numpy(paths[i]["observation"]))
            paths[i]["action"] = expert_actions.detach()
